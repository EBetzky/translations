...
class TranslationService {
  ...
  constructor() {
    ...
    this.dataService
      .initializeIndex(this.indexName, TranslationMappings)
      .then((created) => {
        if (created) {
  ...
  public async getTranslationById(
    id: string,
  ): Promise<Translation | null | undefined> {
    try {
  ...
  public async createTranslation(translationData: Translation) {
    try {
      return await this.dataService.indexDocument(
        this.indexName,
        translationData,
      );
    } catch (error) {
  ...
  private putMockData(mocks: Translation[]) {
    if (mocks.length > 0) {
      ...
      this.LOGGER.info(
        `Feeding index ${this.indexName} with mock data has been completed.`,
      );
    }
  }

  private configureSearchQuery(text: string, type: SearchType): any {
    switch (type) {
