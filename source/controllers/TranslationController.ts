...
class TranslationController {
    ...
    async createTranslation(req: Request, res: Response) {
        ...
        try {
            const translationId = await this.service.createTranslation(translationData);
    ...
    async updateTranslation(req: Request, res: Response) {
        ...
        if (this.isValidInputTranslation(updatedTranslationData)) {
            this.setBadRequestCode(res);
            return;
        }

        try {
            const result = await this.service.updateTranslation(id, updatedTranslationData);
    ...
    async searchTranslations(req: Request, res: Response) {
        ...
        try {
            ...
            const type =
                SearchType[(<string>req.query.type).toUpperCase() as keyof typeof SearchType];
            const translations = await this.service.searchTranslations(term, type, page);
            res.json(translations);
        } catch (error) {
            res.status(500).json({ error: "Failed to retrieve translations" });
        }
    ...
    private isValidInputTranslation(translationData: Translation) {
        return !(
    ...
    private setBadRequestCode(res: Response) {
        res.status(400).json({
            error: "Mandatory fields are timestamp, primaryText, secondaryText"
        });
    }
